...
`generate` first validates all sources, five VS Code results and nine native
...
unmanaged extra theme JSON/portable files fail. It also checks extension contributions
...
## Portable adapters and further work
...
VS Code; `scripts/ports.mjs` translates Windows Terminal fields and Neovim
native highlight groups. `scripts/generate.mjs` orchestrates all outputs.
Neither portable adapter needs the VS Code mapping or manifest. No canonical
token or variant edits were needed to add them.
...
portable token data, now also a standalone terminal export. Phase 4 differentiates
...
and [migration guide](migration.md), without changing theme bytes. Phase 6 adds
three Windows Terminal schemes and three Neovim colorschemes, leaving all four
VS Code outputs byte-locked to v3.3.0. Legacy remains VS Code-only.

`scripts/portable.mjs` validates role-only colors, rejects implicit alpha loss,
composites explicit `over` stacks using the existing encoded-sRGB helper and
checks sixteen distinct ANSI slots with meaningful bright partners. The opaque
native result is a deliberate lossy translation, not an equivalent layered UI.
Neovim maps native captures rather than translating the legacy TextMate profile.
Tests cover inheritance, mapping mutations, source isolation, native names and
byte-identical regeneration. Official schema/document validation and native
Neovim floor/stable smoke are additional hosted gates.

See [ports and the capability matrix](ports.md) for target evidence, installation,
contribution steps, reproducible archives and limits. These artifacts are shipped
in repository/GitHub releases, not the Marketplace extension. No repository
license is chosen.
